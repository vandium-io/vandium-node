...
function processConfig( config ) {
    ...
    if( config.jwt ) {
        ...
        if( jwt.xsrf ) {
            ...
            setEnv( (xsrf.enabled === undefined) ? 'true' : xsrf.enabled, 'VANDIUM_JWT_USE_XSRF' );

            setEnv( xsrf.token, 'VANDIUM_JWT_XSRF_TOKEN_PATH' );
            setEnv( xsrf.claimName, 'VANDIUM_JWT_XSRF_CLAIM_NAME' );
        }
    }
