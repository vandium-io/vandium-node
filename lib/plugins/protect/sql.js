...
const ScanEngine = require( './scan_engine' );
...
function _scan( obj, attacks, attackTypes, mode, filter ) {
    ...
    Object.keys( obj ).forEach( ( key ) => {
        ...
        if( filter( key ) ) {
            ...
            if( utils.isString( value ) ) {
                ...
                _scan( value, attacks, attackTypes, mode, defaultFilter );
            }
        }
    });
}

class SQLScanEngine extends ScanEngine {
    ...
    _doScan( event, filter ) {

        let attacks = Object.keys( SQL_ATTACKS );

        _scan( event, attacks, SQL_ATTACKS, this.mode, filter || defaultFilter );
    }
}


// singleton
module.exports = SQLScanEngine;
