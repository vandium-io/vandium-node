...
class Vandium {
    ...
    configure( config ) {
        ...
        this.lambdaProxy = LambdaProxy.fromConfiguration( config.lambdaProxy );

        let validation = config.validation || {};
        let jwt = config.jwt || {};
        let protect = config.protect || {};
        ...
        if( this.lambdaProxy ) {
            ...
            validation.lambdaProxy = true;
            jwt.lambdaProxy = true;
            protect.lambdaProxy = true;
        }

        prevent.configure();

        this.plugins.validation.configure( validation );
        this.plugins.jwt.configure( jwt);
        this.plugins.protect.configure( protect );
...
Object.defineProperty( Vandium, 'types', {
    ...
    get: function() {
        ...
        return plugins.ValidationPlugin.types;
    }
});
