...
function createProxyObject( body, statusCode, headers ) {
    ...
    proxyObject.headers = utils.clone( headers );
...
function processResult( result, handlerContext ) {
    ...
    let statusCode = result.statusCode || handlerContext.statusCode;
    let headers = result.headers || handlerContext.headers;
...
function processError( error, handlerContext ) {
    ...
    let headers = error.headers || handlerContext.headers;
    ...
    execute( event, context, callback ) {
        ...
        try {
            ...
            let handlerContext = {
                ...
                event,
                context: handlers.safeContext( context ),
