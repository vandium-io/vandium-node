...
class APIHandler extends TypedHandler {
    ...
    constructor( config = {} ) {
        ...
        this._jwt = new JWTValidator( config.jwt );
        this._headers = {};
        this._protection = new Protection( config.protection );
        this.methodHandlers = {};
    ...
    path( p ) {
        ...
        return this._configure( { path: p  }, true );
    }
    ...
    addMethodsToHandler( lambdaHandler ) {
        ...
        this.addlambdaHandlerMethod( 'path', lambdaHandler );
...
constants.HTTP_METHODS.forEach( (methodType) => {
    ...
    APIHandler.prototype[ methodType ] = function( ...args ) {
        ...
        let methods = this._configuration.methods || [];

        methods.push( methodType );

        this.appendConfiguration( { methods } );
