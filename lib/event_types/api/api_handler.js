...
function wantGzip( event ) {

    let headers = event.headers;

    return ( headers && (headers[ 'accept-encoding' ] === 'gzip') );
}
...
class APIHandler extends TypedHandler {
    ...
    processResult( result, context ) {
        ...
        if( body && (this._gzip || (this._gzip === undefined && wantGzip( context.event) )) ) {

            try {

                // encode body as JSON string if not a String or Buffer
                if( !utils.isString( body ) && !Buffer.isBuffer( body ) ) {

                    body = JSON.stringify( body );
                }

                // sync is fine because lambda's run sequentially, body will be a Buffer instance
                body = zlib.gzipSync( body );
                headers[ 'Content-Encoding' ] = 'gzip';
            }
            catch( err ) {

                console.log( 'Cannot gzip response', err );
            }
        }
