...
const { clone, isObject, isString, parseJSON, parseQueryString } = require( '../../utils' );
...
function queryParse( value ) {
    ...
    try {
        ...
        const obj = parseQueryString( value );
        return obj;
...
function processBody( value, qs ) {

    if( isString( value ) ) {

        return ( jsonParse( value ) || ( qs ? queryParse( value ) : null ) || value );
    }
...
class APIHandler extends TypedHandler {
    ...
    constructor( options = {} ) {
        ...
        this._parseQuery = { enabled: options.parseQuery !== undefined ? options.parseQuery : true };
        this._headers = {};
    ...
    formURLEncoded( enabled = true ) {
        ...
        this._parseQuery.enabled = enabled;
    ...
    _validateCurrentMethod() {

        if( !this.currentMethodHandler ) {
    ...
    executePreprocessors( state ) {
        ...
        if( event.body ) {
            ...
            event.body = processBody( event.body, this._parseQuery.enabled );
        }
    ...
    processResult( result, context ) {
        ...
        let headers = clone( result.headers || this._headers );

        if( result.setCookie ) {

            const responseCookie = result.setCookie;

            if( isObject( responseCookie ) ) {

                const { name, value, options } = responseCookie;

                headers[ 'Set-Cookie' ] = cookie.serialize( name, value, options );
            }
            else {
    ...
    async processError( error, context ) {
        ...
        let headers = clone( error.headers || this._headers );
    ...
    _addHandler( type, ...args ) {
        ...
        const methodHandler = createMethodHandler( args );

        this.methodHandlers[ type ] = methodHandler;
        this.currentMethodHandler = methodHandler;
