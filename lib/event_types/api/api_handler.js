...
class APIHandler extends TypedHandler {
    ...
    async processResult( result, context, { method, methodHandler } ) {

        let responseObject = responseProcessor.processResult( result, context, this._headers );

        return await methodHandler.onResponse( responseObject );
    }

    async processError( error, context, { method, methodHandler } ) {
        ...
        let responseObject = responseProcessor.processError( error, this._headers );
    ...
    _addHandler( type, ...args ) {
        ...
        const methodHandler = MethodHandler.create( args );
