...
const { clone, isObject, isObjectEmpty, isString } = require( '../../utils' );
...
function processResult( result, context, additionalHeaders ) {
    ...
    let headers = mergeHeaders( additionalHeaders, result.headers );
...
function processError( error, additionalHeaders ) {
    ...
    let headers = mergeHeaders( additionalHeaders, error.headers );
