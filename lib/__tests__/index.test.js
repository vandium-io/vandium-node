...
const VANDIUM_MODULE_PATH = '../index';
...
describe( 'lib/index', function() {
    ...
    describe( '.createHandler', function() {
      ...
      it( 'apigateway', async function() {

        const innerHandler = sinon.stub().resolves( 'ok' );

        const { createHandler } = vandium;

        const handler = createHandler( ( {apigateway } ) => apigateway( innerHandler ) );

        await LambdaTester( handler )
          .event( (mocks) => mocks.apiGateway().build() )
          .expectResult( (result) => {
