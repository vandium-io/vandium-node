...
vandium.validation = function( schema ) {
	...
	if( schema ) {
        ...
        vandiumInstance.validation.configure( { schema }, true /* schema only */ );
	}
...
vandium.jwt = function() {
    ...
    return vandiumInstance.jwt;
}

vandium.jwt.configure = function( options ) {

    return vandiumInstance.jwt.configure( options );
}

vandium.jwt.enable = function( enable ) {

    return vandiumInstance.jwt.configure( { enable } );
}
